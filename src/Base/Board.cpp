...
void chessBoard::initBoardVertex() {
    ...
    for (i = 0; i < 9; i++) {
        boardVector[++c] = xMargin + xPadding + gap * i;
        boardVector[++c] = yMargin + yPadding + gap * 0;
        boardVector[++c] =  boardVector[c - 2];
        boardVector[++c] = yMargin + yPadding + gap * 8;
    }
    for (i = 0; i < 9; i++) {
        boardVector[++c] = xMargin + xPadding + gap * 0;
        boardVector[++c] = yMargin + yPadding + gap * i;
        boardVector[++c] = xMargin + xPadding + gap * 8;
        boardVector[++c] =  boardVector[c - 2];
    }

    /*-----Set vertex of border in array (72 to 79 for 4 corners)--------*/
    boardVector[++c] = xMargin;
    boardVector [++c] = yMargin;
    boardVector[++c] = xMargin;
    boardVector [++c] = yMargin + yPadding + gap * 8 + yPadding;
    boardVector[++c] = xMargin + xPadding + gap * 8 + xPadding;
    boardVector[++c] = yMargin + yPadding + gap * 8 + yPadding;
    boardVector[++c] = xMargin + xPadding + gap * 8 + xPadding;
    boardVector[++c] = yMargin;
    ...
    for (j = 0; j < 8; j++) {
        for (i = 0; i < 8; i++) {
            boardVector[++c] = xMargin + xPadding + boardLineWidth / 2 + gap * i;
            boardVector[++c] = yMargin + yPadding + boardLineWidth / 2 + gap * j;
            boardVector[++c] = xMargin + xPadding + boardLineWidth / 2 + gap * i;
            boardVector[++c] = yMargin + yPadding - boardLineWidth / 2 + gap * (j + 1);
            boardVector[++c] = xMargin + xPadding - boardLineWidth / 2 + gap * (i + 1);
            boardVector[++c] = yMargin + yPadding - boardLineWidth / 2 + gap * (j + 1);
            boardVector[++c] = xMargin + xPadding - boardLineWidth / 2 + gap * (i + 1);
            boardVector[++c] = yMargin + yPadding + boardLineWidth / 2 + gap * j;
        }
